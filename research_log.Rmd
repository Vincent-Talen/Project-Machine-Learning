...
```{r _mean plots, fig.width = 7.5, fig.height = 9.5, fig.cap = "Split violin plots joined with box plots of 'mean' feature columns"}
# Create split violin plots for all mean feature columns
createAndArrangePlots("_mean")
```

```{r _se plots, fig.width = 7.5, fig.height = 9.5, fig.cap = "Split violin plots joined with box plots of 'standard error' feature columns"}
# Create split violin plots for all standard error feature columns
createAndArrangePlots("_se")
```

```{r _worst plots, fig.width = 7.5, fig.height = 9.5, fig.cap = "Split violin plots joined with box plots of 'worst/extreme' feature columns"}
...
The couple of columns that stand out the most are the `Radius`, `Perimeter`, `Area`, `Concave Points` and `Radius` feature columns, these all seem to have a clear distinction between the diagnosis classification.  
...
\newpage
### Bivariate analysis
To identify the correlations between features and visualize them, the perfect way is to create a heatmap. It visualizes the strength of the correlation between all columns, so all possible combinations can be seen. The heatmap takes a correlation matrix and displays its values with colors so correlations can easily be visually seen.
...
# Insert column with feature column names and set is as the left most column
cor_matrix <- cor_matrix %>% 
    mutate(col_names = all_of(column_names)) %>% 
    select(31, 1:30)
# Convert data to long format
cor_matrix_long <- pivot_longer(data = cor_matrix, cols = all_of(column_names), 
                                names_to = "variable", values_to = "cor")
```


```{r heatmap, fig.cap = "Heatmap showing pairwise correlation between feature colums"}
